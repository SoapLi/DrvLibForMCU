...
cJSON *root, *psub;
	...
	if (root != NULL) {
		...
		if ((locat != NULL)&&(now != NULL)&&(last_update !=NULL))
		{
			...
			// lv_label_set_text(label4,weathe.cit);
			// lv_label_set_text(label5,weathe.weather_text);
			// lv_label_set_text(label6,weathe.temperatur);
			// lv_label_set_text(label8,weathe.time_last_update);



		//	display_string_8x16(1, 1, (uint8_t*)weathe.temperatur);
