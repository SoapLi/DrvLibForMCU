...
/* 存放get请求的相关数据 */
static uint8_t https_get_buffer[1024 + 200];
/* 存放接收到GET请求的数据 */
 ...
 *               Ver0.0.1:
                     Helon_Chan, 2018/06/27, 初始化版本\n 
 */
static esp_err_t http_url_parse(char *URL, char *mhost, char *mfilename)
{
	char *PA;
	char *PB;
	/* 分别判断传来的实参是不是为空 */
	...
	os_memset(mhost, 0, sizeof(host) / sizeof(char));
	os_memset(mfilename, 0, sizeof(filename) / sizeof(char));
	PA = URL;
	...
	if (PB)
	{
		...
		os_memcpy(mhost, PA, os_strlen(PA) - os_strlen(PB));
		if (PB + 1)
		{
			os_memcpy(mfilename, PB + 1, os_strlen(PB - 1));
			mfilename[os_strlen(PB) - 1] = 0;
		}
		mhost[os_strlen(PA) - os_strlen(PB)] = 0;
	}
	/* 如果传进来的网址是主页则直接获取主页地址内容 */
	else
	{
		os_memcpy(mhost, PA, os_strlen(PA));
		mhost[os_strlen(PA)] = 0;
	}
	return ESP_OK;
}
...
static int16_t tls_client_handle(uint8_t method)
{
	...
	if (method == GET_REQ)
	{
		len = sprintf((char *)https_get_buffer, USR_HTTPS_GET, filename, host);
	}
	else
	{
		len = sprintf((char *)https_get_buffer, USR_HTTPS_POST, filename, strlen(POST_CONTENT), host, POST_CONTENT);
	}
